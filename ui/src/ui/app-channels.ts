...
export async function handleWhatsAppStart(host: wehelperApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: wehelperApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: wehelperApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: wehelperApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: wehelperApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: wehelperApp): string {
...
export function handleNostrProfileCancel(host: wehelperApp) {
...
export function handleNostrProfileToggleAdvanced(host: wehelperApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: wehelperApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: wehelperApp) {
  const state = host.nostrProfileFormState;
