  ...
  return html`
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          <div class="card-title">${t("Connected Instances", "已连接实例")}</div>
          <div class="card-sub">${t("Presence beacons from the gateway and clients.", "来自网关与客户端的在线信标。")}</div>
        </div>
      ...
      <div class="list" style="margin-top: 16px;">
        ${
          props.entries.length === 0
            ? html`
                <div class="muted">${t("No instances reported yet.", "暂无实例上报。")}</div>
              `
...
function renderEntry(entry: PresenceEntry) {
  const lastInput =
    entry.lastInputSeconds != null
      ? t(`${entry.lastInputSeconds}s ago`, `${entry.lastInputSeconds} 秒前`)
      : t("n/a", "暂无");
  ...
  return html`
    <div class="list-item">
      <div class="list-main">
        <div class="list-title">${entry.host ?? t("unknown host", "未知主机")}</div>
      ...
      <div class="list-meta">
        ...
        <div class="muted">${t("Last input", "最近输入")} ${lastInput}</div>
        <div class="muted">${t("Reason", "原因")} ${entry.reason ?? ""}</div>
      </div>
    </div>
  `;
}
