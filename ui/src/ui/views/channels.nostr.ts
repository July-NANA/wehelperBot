...
export function renderNostrCard(params: {
    ...
    return html`
      <div class="account-card">
        ...
        <div class="status-list account-card-status">
          ...
          <div>
            <span class="label">${t("Public Key", "公钥")}</span>
    ...
    return html`
      <div style="margin-top: 16px; padding: 12px; background: var(--bg-secondary); border-radius: 8px;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
          <div style="font-weight: 500;">${t("Profile", "资料")}</div>
          ${
            summaryConfigured
              ? html`
                <button
          ...
          hasAnyProfileData
            ? html`
              <div class="status-list">
                ...
                ${name ? html`<div><span class="label">${t("Name", "名称")}</span><span>${name}</span></div>` : nothing}
                ${
                  displayName
                    ? html`<div><span class="label">${t("Display Name", "显示名")}</span><span>${displayName}</span></div>`
                    : nothing
                }
                ${
                  about
                    ? html`<div><span class="label">${t("About", "简介")}</span><span style="max-width: 300px; overflow: hidden; text-overflow: ellipsis;">${about}</span></div>`
                    : nothing
                }
                ${nip05 ? html`<div><span class="label">${t("NIP-05", "NIP-05")}</span><span>${nip05}</span></div>` : nothing}
              </div>
            `
            : html`
                <div style="color: var(--text-muted); font-size: 13px">
                  ${t('No profile set. Click "Edit Profile" to add your name, bio, and avatar.', "未设置资料。点击“编辑资料”添加名称、简介和头像。")}
                </div>
              `
        }
      </div>
  ...
  return html`
    <div class="card">
      ...
      <div class="card-sub">${t("Decentralized DMs via Nostr relays (NIP-04).", "通过 Nostr 中继进行去中心化私信（NIP-04）。")}</div>
      ${accountCountLabel}
        ...
        hasMultipleAccounts
          ...
          : html`
            <div class="status-list" style="margin-top: 16px;">
              <div>
                <span class="label">${t("Configured", "已配置")}</span>
                <span>${summaryConfigured ? t("Yes", "是") : t("No", "否")}</span>
              </div>
              <div>
                <span class="label">${t("Running", "运行中")}</span>
                <span>${summaryRunning ? t("Yes", "是") : t("No", "否")}</span>
              </div>
              <div>
                <span class="label">${t("Public Key", "公钥")}</span>
              ...
              <div>
                ...
                <span>${summaryLastStartAt ? formatAgo(summaryLastStartAt) : t("n/a", "暂无")}</span>
              </div>
            </div>
          `
      }
      ...
      <div class="row" style="margin-top: 12px;">
        <button class="btn" @click=${() => props.onRefresh(false)}>${t("Refresh", "刷新")}</button>
      </div>
    </div>
  `;
}
